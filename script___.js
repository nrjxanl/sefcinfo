...
$("#menu > div").hover(
    function() {
        $(this).find("span").css("background", "#000060")
    }, 
    function() {
...
function resetAllTextStyles() {
    if (!timeMapping) return; // timeMapping이 설정되지 않은 경우를 대비
    const allLyrics = document.querySelectorAll('p[id^="lyrics-"]');
    allLyrics.forEach(el => {
        el.style.color = "#05090a";
        el.style.fontWeight = "300";
        el.querySelectorAll("ruby, rb, rp, rt").forEach(innerEl => {
            innerEl.style.color = "#05090a";
            innerEl.style.fontWeight = "300";
        });
    });
}

function updateTextStyle(currentTime) {
    if (!timeMapping || timeMapping.length === 0) return;
    
    // 시간순으로 정렬된 가사 배열의 뒤에서부터 탐색하여,
    // 현재 재생 시간보다 작거나 같은(이미 지나간) 가장 마지막 가사를 찾습니다.
    const currentMapping = [...timeMapping].reverse().find(({ time }) => currentTime >= time);

    // 지나간 가사가 하나도 없는 경우 (영상이 막 시작한 경우)
    if (!currentMapping) {
        // 이전에 볼드 처리된 것이 있다면 초기화
        if (lastBoldId) {
            resetAllTextStyles();
            lastBoldId = null;
        }
    ...
    if (lastBoldId !== currentMapping.elementId) {
        if (lastBoldId) {
...
if ($("#team").length || $("#chantsName").length) {
    ...
    let chantsName, chantsIdx;

    for (i = 0; i < Object.keys(teamChants).length; i++) {
        $("#team").append("<div glass='true' transl='y'><p>" + teamChants[i] + "</p><div></div></div>")
    }

    for (i = 0; i < Object.keys(playersChants).length; i++) {
        $("#players").append("<div glass='true' transl='y'><p>" + playersChants[i] + "</p><div></div></div>")
    }

    $("#team, #players").children("div").on("click", function () {

        text = $(this).find("p").html().replace(/<span[^>]*>.*?<\/span>/gi, "");

        // 이전에 실행 중이던 인터벌(타이머)이 있다면 새로운 곡을 로드하기 전에 반드시 중지
        if (intervalId) {
            clearInterval(intervalId);
            intervalId = null;
        }
        // 이전에 볼드 처리된 가사 기록 초기화
        lastBoldId = null;

        $("#team, #players").find("div > div").slideUp(400);

        if (chantsName !== text) {
            chantsName = text;
            chantsIdx = $(this).index();

            $("#team > div > div, #players > div > div").empty();

            $(this).find("div").append("<div id='chantsVideo'></div>")

            addLyrics(chantsName, chantsIdx);

            $(this).find("div").slideDown(400);
        } else {
            // 같은 요소를 다시 클릭해서 닫는 경우를 위해 추가
            chantsName = null;
        }
    });
...
function addLyrics(name, idx) {
    ...
    if (name == "SEFC 콜") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>짝 짝 짝 짝</p><p id='lyrics-2'>SEFC</p>")
    }
    ...
    if (name == "서울 이랜드 콜(1)") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>짝 짝짝 짝짝</p><p id='lyrics-2'>서울 이랜드</p>")
    }
    ...
    if (name == "서울 이랜드 콜(2)") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>서울 이랜드 서울 이랜드</p>")
    }
    ...
    if (name == "2 3 4 서울 콜") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>짝짝 짝짝짝 <ruby><rb>짝짝짝짝</rb><rp>(</rp><rt>알레알레</rt><rp>)</rp></ruby></p><p id='lyrics-2'>서울</p>")
    }

    if (name == "3 3 4 2 서울 콜") {
        url = "fRxMNt_J2Ts"
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>서울 서울 서울 오</p><p id='lyrics-2'>짝짝짝 짝짝짝 짝짝짝짝 짝짝</p><p id='lyrics-3'>짝짝짝 짝짝짝 짝짝짝짝 짝짝</p>")
    }
    ...
    if (name == "샤랄라 SEFC") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>샤랄라랄라랄라 SEFC</p>")
    }
    ...
    if (name == "하나되어") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>하나되어 나아가자</p><p id='lyrics-2'>깃발을 높이 들고</p><p id='lyrics-3'>우리의 푸른 함성으로</p><p id='lyrics-4'>승리를 노래하자</p><p id='lyrics-5'>라라라라 라라라라</p><p id='lyrics-6'>라라라 라라라라</p><p id='lyrics-7'>라라라 라라 라라라라</p><p id='lyrics-8'>라라라 라라라라</p>")
    }
    ...
    if (name == "세트피스 콜") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>골 골 골 골</p><p id='lyrics-2'>서울 이랜</p>")
    }
    ...
    if (name == "뛰어 뛰어 서울 콜") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>뛰어 뛰어 뛰어 뛰어 서울</p>")
    }
    ...
    if (name == "서울의 노래") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>서울의 승리를 위해 함께 외치자</p><p id='lyrics-2'>오오오 오 오오 오 오</p><p id='lyrics-3'>오오오 오 오오 오 오오</p><p id='lyrics-4'>승리 위한 우리 노래가 여기에 울려 퍼지면</p><p id='lyrics-5'>끝까지 맞서 싸워 서울은 승리하리라</p><p id='lyrics-6'>서울</p>")
    }
    ...
    if (name == "사랑해 서울 이랜드") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>사랑해 서울 이랜드 사랑해 서울 이랜드</p><p id='lyrics-2'>사랑해 서울 이랜드 오오</p>")
    }
    ...
    if (name == "한강에서부터 제주까지") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>한강에서부터 <ruby><rb>◌◌</rb><rp>(</rp><rt>지역명</rt><rp>)</rp></ruby>까지</p><p id='lyrics-2'>우리는 싸운다 SEFC</p><p id='lyrics-3'>오오오오 오오오오</p><p id='lyrics-4'>에에에에 에에에에</p>")
    }
    ...
    if (name == "표범의 전사들") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>알레알레오 서울 이랜드 표범의 전사들이여</p><p id='lyrics-2'>맞서 싸워라 승리하리라 저 높은 곳에 우뚝 서리라</p><p id='lyrics-3'>오 오오오 오오오오 오오오오 오오오오</p><p id='lyrics-4'>오 오오오 오오오오 오오오오 오오오오</p>")
    }
    ...
    if (name == "앞으로 가자") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>우리들은 서울 이랜드</p><p id='lyrics-2'>서울 이랜 승리 위해 앞으로 가자</p><p id='lyrics-3'>우리들은 서울 이랜드</p><p id='lyrics-4'>서울 이랜 승리 위해 앞으로 가자</p><p id='lyrics-5'>알레알레 알레알레오</p><p id='lyrics-6'>알레알레 알레알레 알레알레오</p><p id='lyrics-7'>알레알레 알레알레오</p><p id='lyrics-8'>알레알레 알레알레 알레알레오</p>")
    }
    ...
    if (name == "서울의 아리아") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>라 라라라라 라라 라 라라라라 라라</p><p id='lyrics-2'>라 라라라라 라라 라 라라 라라라</p><p id='lyrics-3'>파랗게 피어나는 승리를 위한 함성</p><p id='lyrics-4'>서울의 영원한 폭풍이 되리라</p>")
    }
    ...
    if (name == "너를 태우고") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>푸른색이 빛도는 나의 서울이여</p><p id='lyrics-2'>황금빛 저녁놀 우리들의 청춘</p><p id='lyrics-3'>푸른색이 빛도는 나의 서울이여</p><p id='lyrics-4'>진격하라 저 앞으로 승리를 위해</p><p id='lyrics-5'>라라 라라 라라라 라라라 라라라</p><p id='lyrics-6'>라라라 라라라 라라 라라 라라</p><p id='lyrics-7'>라라 라라 라라라 라라라 라라라</p><p id='lyrics-8'>라라라라 라라라라 라라 라라라</p>")
    }
    ...
    if (name == "페파스") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>서울 승리 위해 외쳐 우리는 끝까지 노래하고 뛰어</p><p id='lyrics-2'>후 하</p><p id='lyrics-3'>서울 승리 위해 외쳐 우리는 끝까지 노래하고 뛰어</p><p id='lyrics-4'>후 하</p><p id='lyrics-5'>라라 라라라라 라라 라라라라 라라 라라라라 라라</p><p id='lyrics-6'>서울</p><p id='lyrics-7'>라라 라라라라 라라 라라라라 라라 라라라라 라라</p>")
    }
    ...
    if (name == "우리는 항상 여기에") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>우리는 항상 여기에 언제나 너희를 위해</p><p id='lyrics-2'>저 푸른 소나무같이 우린 여기에</p>")
    }
    ...
    if (name == "뱃놀이") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>즐거운 서울 승리의 날에 <ruby><rb>◌◌</rb><rp>(</rp><rt>상대</rt><rp>)</rp></ruby> 팬 우는 소리</p><p id='lyrics-2'>지고 가는 <ruby><rb>◌◌</rb><rp>(</rp><rt>상대</rt><rp>)</rp></ruby> 팬이 처량도 하구나</p><p id='lyrics-3'>어기야 디여엉차 어기영차 어기야 디여엉 어기영차</p><p id='lyrics-4'>뱃놀이 가잔다</p>")
    }
    ...
    if (name == "우리 모두 다 같이") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>우리 모두 다 같이 서울 서울</p><p id='lyrics-2'>다시 한 번 더 크게 서울 서울</p><p id='lyrics-3'>라라라라 라라라라 라라라라 라라라</p><p id='lyrics-4'>나의 사랑 서울 이랜드</p><p id='lyrics-5'>라라라라 라라라라 라 라라</p><p id='lyrics-6'>라라라라 라라라라 라 라라</p><p id='lyrics-7'>라라라라 라라라라 라라라라 라라라</p><p id='lyrics-8'>나의 사랑 서울 이랜드</p>")
    }
    ...
    if (name == "사랑한다 서울") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>사랑한다 서울 사랑한다 이랜</p><p id='lyrics-2'>내 가슴속에 영원히 남을 사랑이 되어라</p><p id='lyrics-3'>사랑한다 서울 사랑한다 이랜</p><p id='lyrics-4'>내 가슴속에 영원히 남을 사랑이어라</p>")
    }
    ...
    if (name == "승리하라 서울 콜") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>짝짝짝짝 짝짝</p><p id='lyrics-2'>승리하라 서울</p>")
    }
    ...
    if (name == "우리는 서울 이랜드") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>오오오오 오오오오</p><p id='lyrics-2'>우리는 서울 이랜드</p><p id='lyrics-3'>오오오오 오오 오오</p><p id='lyrics-4'>승리의 서울 이랜드</p>")
    }
    ...
    if (name == "좌로 우로") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>푸른 도시 위를 질주하여라</p><p id='lyrics-2'>피 끓는 심장이 터질 때까지</p><p id='lyrics-3'>승리 위해 뛰어 저 표범처럼</p><p id='lyrics-4'>좌로 우로 뛰면서 서울을 외치자</p><p id='lyrics-5'>좌로 뜨드 뜨드 뜨드</p><p id='lyrics-6'>우로 뜨드 뜨드 뜨드</p><p id='lyrics-7'>좌로 뜨드 뜨드 뜨드</p><p id='lyrics-8'>우로 뜨드 뜨드 뜨드</p><p id='lyrics-9'>뜨뜨드드드드 뜨뜨드드드</p><p id='lyrics-10'>뜨뜨드드드드 뜨드드뜨드드</p><p id='lyrics-11'>뜨뜨드드드드 뜨뜨드드드</p><p id='lyrics-12'>뜨드뜨드드뜨드드 뜨드드뜨드드</p>")
    }
    ...
    if (name == "둥글게 둥글게") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>승리를 위하여 다 함께 일어나</p><p id='lyrics-2'>서울 이랜 승리 위해 함께 외치자</p><p id='lyrics-3'>깃발을 흔들며 북 소리 맞춰서</p><p id='lyrics-4'>라라라라 머플러를 힘껏 돌리자</p><p id='lyrics-5'>알레 알레 알레 서울 이랜드</p><p id='lyrics-6'>알레 알레 알레 서울 이랜드</p><p id='lyrics-7'>함께 나아가자 포르차 서울</p><p id='lyrics-8'>하나되어 불러 봅시다</p>")
    }
    ...
    if (name == "포에버") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>오 나의 서울 오 나의 서울</p><p id='lyrics-2'>어떠한 시련들이 다가와도</p><p id='lyrics-3'>우리는 항상 이곳을 지킨다네</p><p id='lyrics-4'>영광의 날이 다가올 때까지</p><p id='lyrics-5'>우리의 사랑 영원히 변함없네</p>")
    }
    ...
    if (name == "항상 사랑할 거야") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>가자 가자 서울은 승리하리라</p><p id='lyrics-2'>이 목소리는 멈추지 않을 거야</p><p id='lyrics-3'>너희가 어디 있든 따라가리라</p><p id='lyrics-4'>항상 사랑할 거야</p>")
    }
    ...
    if (name == "푸른 심장") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>그대와 이 길을 같이 걷는다면</p><p id='lyrics-2'>나는 그 누구도 두렵지가 않네</p><p id='lyrics-3'>우리의 가슴에 불을 지핀다면</p><p id='lyrics-4'>나는 그 누구도 부럽지가 않네</p><p id='lyrics-5'>라라라라</p><p id='lyrics-6'>자 가자 가자 앞으로</p><p id='lyrics-7'>터뜨려 너의 푸른 심장을</p><p id='lyrics-8'>자 가자 가자 앞으로</p><p id='lyrics-9'>터뜨려 너의 푸른 심장</p><p id='lyrics-10'>라라라라</p>")
    }
    ...
    if (name == "서울 승리를 향해") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>서울 서울 승리를 향해</p><p id='lyrics-2'>언젠가는 찾아오리 영광의 그날</p><p id='lyrics-3'>서울 서울 승리를 향해</p><p id='lyrics-4'>별을 다는 순간까지 모두 외쳐라</p>")
    }

    if (name == "We Cheer for Seoul") {
        url = "rEkqhK96icg"
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>서울 서울 갖다 박아 버려</p><p id='lyrics-2'>서울 서울 때려 부숴 버려</p><p id='lyrics-3'>서울 서울 갖다 박아 버려</p><p id='lyrics-4'>서울 서울 때려 부숴 버려</p><p id='lyrics-5'>올레 올레오 서울 이랜</p><p id='lyrics-6'>올레 올레오 서울 이랜</p><p id='lyrics-7'>올레 올레오 서울 이랜</p><p id='lyrics-8'>올레 올레오 서울 이랜</p>")
    }
    ...
    if (name == "서울을 위해 달려라") {
        ...
        $(`#team > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>달려라 달려라 서울을 위해</p><p id='lyrics-2'>흐르는 땀을 바람에 흩뿌리며</p><p id='lyrics-3'>힘들고 숨이 차 넘어져도</p><p id='lyrics-4'>또 다시 일어나 앞으로 나아가리</p><p id='lyrics-5'>라라라 라라라 라라라 라라</p><p id='lyrics-6'>라라라 라라라라라라 라라라라</p><p id='lyrics-7'>라라라 라라라 라라라 라라</p><p id='lyrics-8'>라라라 라라라라라라 라 라라</p>")
    }
    ...
    if (name == "캡틴 콜") {
        ...
        $(`#players > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>라라라라 라라라라 캡틴 <ruby><rb>◌◌</rb><rp>(</rp><rt>선수</rt><rp>)</rp></ruby></p><p id='lyrics-2'>라라라라 라라라라 캡틴 <ruby><rb>◌◌</rb><rp>(</rp><rt>선수</rt><rp>)</rp></ruby></p><p id='lyrics-3'>승리 위해 다 함께 외치자</p><p id='lyrics-4'>라라라라 라라라라 캡틴 <ruby><rb>◌◌</rb><rp>(</rp><rt>선수</rt><rp>)</rp></ruby></p>")
    }
    ...
    if (name == "득점 콜") {
        ...
        $(`#players > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>오레오 오레오 서울의 <ruby><rb>◌◌</rb><rp>(</rp><rt>선수</rt><rp>)</rp></ruby></p><p id='lyrics-2'>오레오 오레오 승리는 우리의 것</p>")
    }
    ...
    if (name == "오스마르 콜(1)") {
        ...
        $(`#players > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>오 오 오스마르</p><p id='lyrics-2'>오오오 오오오 오오오 오오오</p>")
    }
    ...
    if (name == "오스마르 콜(2)") {
        ...
        $(`#players > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>오스마르 오스마르 서울만을 사랑해</p><p id='lyrics-2'>레울파크에 왔네 오 오 오스마르 헤이</p><p id='lyrics-3'>오스마르 오스마르 스페인의 검투사</p><p id='lyrics-4'>승격을 위해 왔네 오 오 오스마르 헤이</p><p id='lyrics-5'>오스마르 오스마르 이바녜스 바르바</p><p id='lyrics-6'>황새도 막지 못해 오 오 오스마르 헤이</p>")
    }
    ...
    if (name == "변경준 콜") {
        ...
        $(`#players > div:nth-of-type(${idx}) > div`).append("<p id='lyrics-1'>치고 달려나가 서울 변경준</p><p id='lyrics-2'><ruby><rb>◌◌</rb><rp>(</rp><rt>상대</rt><rp>)</rp></ruby> 부숴 버려 서울 변경준</p><p id='lyrics-3'>멋지구나 잘생겼다 잔디 위의 푸른 야수</p><p id='lyrics-4'>스피드가 장난 아니지</p>")
    }

    player = new YT.Player('chantsVideo', {
        height: '100%',
        width: '100%',
        videoId: url,
        events: {
...
function transl() {
    ...
    //     "강지후": ["Kang Jihoo", "Kang Jihoo", "Kang Jihoo", "カン・チフ"],
    //     "권민서": ["Kwon Minseo", "Kwon Minseo", "Kwon Minseo", "クォン・ミンソ"],
    //     "권용현": ["Kwon Yonghyun", "Kwon Yonghyun", "Kwon Yonghyun", "クォン・ヨンヒョン"],
    //     "권재윤": ["Kwon Jaeyoon", "Kwon Jaeyoon", "Kwon Jaeyoon", "クォン・チェユン"],
    //     "김강민1": ["Kim Kangmin", "Kim Kangmin", "Kim Kangmin", "キム・カンミン"],
    //     "김강민2": ["Kim Kangmin", "Kim Kangmin", "Kim Kangmin", "キム・カンミン"],
    //     "김건우": ["Kim Kunwoo", "Kim Kunwoo", "Kim Kunwoo", "キム・コヌ"],
    //     "김경욱": ["Kim Gyeonguk", "Kim Gyeonguk", "Kim Gyeonguk", "キム・キョンウク"],
    //     "김규민": ["Kim Gyumin", "Kim Gyumin", "Kim Gyumin", "キム・キュミン"],
    //     "김규연": ["Kim Gyuyeon", "Kim Gyuyeon", "Kim Gyuyeon", "キム・キュヨン"],
    //     "김명관": ["Kim Myunggwan", "Kim Myunggwan", "Kim Myunggwan", "キム・ミョングァン"],
    //     "김명식": ["Kim Myungsik", "Kim Myungsik", "Kim Myungsik", "キム・ミョンシク"],
    //     "김민겸": ["Kim Minkyum", "Kim Minkyum", "Kim Minkyum", "キム・ミンギョム"],
    //     "김민성": ["Kim Minsung", "Kim Minsung", "Kim Minsung", "キム・ミンソン"],
    //     "김민우": ["Kim Minwoo", "Kim Minwoo", "Kim Minwoo", "キム・ミヌ"],
    //     "김민재": ["Kim Minjae", "Kim Minjae", "Kim Minjae", "キム・ミンジェ"],
    //     "김범규": ["Kim Beomgyu", "Kim Beomgyu", "Kim Beomgyu", "キム・ポムギュ"],
    //     "김서준": ["Kim Seojoon", "Kim Seojoon", "Kim Seojoon", "キム・ソジュン"],
    //     "김서진": ["Kim Seojin", "Kim Seojin", "Kim Seojin", "キム・ソジン"],
    //     "김성빈": ["Kim Soungbin", "Kim Soungbin", "Kim Soungbin", "キム・ソンビン"],
    //     "김성연": ["Kim Sungyeon", "Kim Sungyeon", "Kim Sungyeon", "キム・ソンヨン"],
    //     "김예찬": ["Kim Yechan", "Kim Yechan", "Kim Yechan", "キム・イェチャン"],
    //     "김우진": ["Kim Woojin", "Kim Woojin", "Kim Woojin", "キム・ウジン"],
    //     "김우현": ["Kim Woohyun", "Kim Woohyun", "Kim Woohyun", "キム・ウヒョン"],
    //     "김윤건": ["Kim Yungeon", "Kim Yungeon", "Kim Yungeon", "キム・ユンゴン"],
    //     "김정민": ["Kim Jungmin", "Kim Jungmin", "Kim Jungmin", "キム・チョンミン"],
    //     "김주훈": ["Kim Joohoon", "Kim Joohoon", "Kim Joohoon", "キム・チュフン"],
    //     "김준서": ["Kim Joonseo", "Kim Joonseo", "Kim Joonseo", "キム・チュンソ"],
    //     "김지민": ["Kim Jimin", "Kim Jimin", "Kim Jimin", "キム・チミン"],
    //     "김지완1": ["Kim Jiwan", "Kim Jiwan", "Kim Jiwan", "キム・チワン"],
    //     "김지완2": ["Kim Jiwan", "Kim Jiwan", "Kim Jiwan", "キム・チワン"],
    //     "김지호": ["Kim Jiho", "Kim Jiho", "Kim Jiho", "キム・チホ"],
    //     "김태호": ["Kim Taeho", "Kim Taeho", "Kim Taeho", "キム・テホ"],
    //     "김태황": ["Kim Taehwang", "Kim Taehwang", "Kim Taehwang", "キム・テファン"],
    //     "김태훈": ["Kim Taehun", "Kim Taehun", "Kim Taehun", "キム・テフン"],
    //     "김한빈": ["Kim Hanbeen", "Kim Hanbeen", "Kim Hanbeen", "キム・ハンビン"],
    //     "김한솔": ["Kim Hansol", "Kim Hansol", "Kim Hansol", "キム・ハンソル"],
    //     "김현석": ["Kim Hyunsuk", "Kim Hyunsuk", "Kim Hyunsuk", "キム・ヒョンソク"],
    //     "김현수": ["Kim Hyunsu", "Kim Hyunsu", "Kim Hyunsu", "キム・ヒョンス"],
    //     "김호준2": ["Kim Hojun", "Kim Hojun", "Kim Hojun", "キム・ホジュン"],
    //     "노규민": ["Noh Kyumin", "Noh Kyumin", "Noh Kyumin", "ノ・キュミン"],
    //     "노윤철": ["Noh Yuncheol", "Noh Yuncheol", "Noh Yuncheol", "ノ・ユンチョル"],
    //     "라호영": ["Rha Hoyeong", "Rha Hoyeong", "Rha Hoyeong", "ラ・ホヨン"],
    //     "류예찬": ["Ryu Yechan", "Ryu Yechan", "Ryu Yechan", "リュ・イェチャン"],
    //     "문성욱": ["Moon Sunguk", "Moon Sunguk", "Moon Sunguk", "ムン・ソンウク"],
    //     "문우주": ["Moon Wooju", "Moon Wooju", "Moon Wooju", "ムン・ウジュ"],
    //     "문종보": ["Moon Jongbo", "Moon Jongbo", "Moon Jongbo", "ムン・チョンボ"],
    //     "문현서": ["Moon Hyeonseo", "Moon Hyeonseo", "Moon Hyeonseo", "ムン・ヒョンソ"],
    //     "박민규": ["Park Mingyu", "Park Mingyu", "Park Mingyu", "パク・ミンギュ"],
    //     "박서준": ["Park Seojun", "Park Seojun", "Park Seojun", "パク・ソジュン"],
    //     "박세준": ["Park Sejun", "Park Sejun", "Park Sejun", "パク・セジュン"],
    //     "박세훈": ["Park Sehun", "Park Sehun", "Park Sehun", "パク・セフン"],
    //     "박정근": ["Park Jeongkun", "Park Jeongkun", "Park Jeongkun", "パク・チョングン"],
    //     "박준성1": ["Park Junsung", "Park Junsung", "Park Junsung", "パク・チュンソン"],
    //     "박찬솔": ["Park Chansol", "Park Chansol", "Park Chansol", "パク・チャンソル"],
    //     "방지환": ["Bang Jihwan", "Bang Jihwan", "Bang Jihwan", "パン・チファン"],
    //     "배지민": ["Bae Jimin", "Bae Jimin", "Bae Jimin", "ペ・チミン"],
    //     "백성민": ["Baek Sungmin", "Baek Sungmin", "Baek Sungmin", "ペク・ソンミン"],
    //     "성승훈": ["Sung Seunghoon", "Sung Seunghoon", "Sung Seunghoon", "ソン・スンフン"],
    //     "소승민": ["So Seungmin", "So Seungmin", "So Seungmin", "ソ・スンミン"],
    //     "송동형": ["Song Donghyung", "Song Donghyung", "Song Donghyung", "ソン・トンヒョン"],
    //     "송준엽": ["Song Junyeop", "Song Junyeop", "Song Junyeop", "ソン・チュニョプ"],
    //     "송지석": ["Song Jiseok", "Song Jiseok", "Song Jiseok", "ソン・チソク"],
    //     "신동찬": ["Sin Dongchan", "Sin Dongchan", "Sin Dongchan", "シン・トンチャン"],
    //     "신정우": ["Shin Jungwoo", "Shin Jungwoo", "Shin Jungwoo", "シン・チョンウ"],
    //     "신준석": ["Shin Junseok", "Shin Junseok", "Shin Junseok", "シン・チュンソク"],
    //     "안광윤": ["Ahn Gwangyun", "Ahn Gwangyun", "Ahn Gwangyun", "アン・クァンユン"],
    //     "안연우": ["An Yeonwoo", "An Yeonwoo", "An Yeonwoo", "アン・ヨヌ"],
    //     "양기찬": ["Yang Kichan", "Yang Kichan", "Yang Kichan", "ヤン・キチャン"],
    //     "양승민": ["Yang Seungmin", "Yang Seungmin", "Yang Seungmin", "ヤン・スンミン"],
    //     "양윤석": ["Yang Yoonsuk", "Yang Yoonsuk", "Yang Yoonsuk", "ヤン・ユンソク"],
    //     "양재민": ["Yang Jeamin", "Yang Jeamin", "Yang Jeamin", "ヤン・チェミン"],
    //     "엄류진": ["Eom Ryujin", "Eom Ryujin", "Eom Ryujin", "オム・リュジン"],
    //     "여재민": ["Yeo Jaemin", "Yeo Jaemin", "Yeo Jaemin", "ヨ・チェミン"],
    //     "오우진": ["Oh Woojin", "Oh Woojin", "Oh Woojin", "オ・ウジン"],
    //     "오정주": ["Oh Jeongju", "Oh Jeongju", "Oh Jeongju", "オ・チョンジュ"],
    //     "오청현": ["Oh Cheonghyeon", "Oh Cheonghyeon", "Oh Cheonghyeon", "オ・チョンヒョン"],
    //     "오현택": ["Oh Hyuntaek", "Oh Hyuntaek", "Oh Hyuntaek", "オ・ヒョンテク"],
    //     "원호준": ["Won Hojun", "Won Hojun", "Won Hojun", "ウォン・ホジュン"],
    //     "유영준": ["Yu Youngjoon", "Yu Youngjoon", "Yu Youngjoon", "ユ・ヨンジュン"],
    //     "유예찬": ["Yu Yechan", "Yu Yechan", "Yu Yechan", "ユ・イェチャン"],
    //     "윤진서": ["Yun Jinseo", "Yun Jinseo", "Yun Jinseo", "ユン・チンソ"],
    //     "음재식": ["Eum Jaesik", "Eum Jaesik", "Eum Jaesik", "ウム・チェシク"],
    //     "이규섭": ["Lee Kyuseop", "Lee Kyuseop", "Lee Kyuseop", "イ・キュソプ"],
    //     "이우진": ["Lee Woojin", "Lee Woojin", "Lee Woojin", "イ・ウジン"],
    //     "이윤재": ["Lee Yunjae", "Lee Yunjae", "Lee Yunjae", "イ・ユンジェ"],
    //     "이재웅": ["Lee Jaewoong", "Lee Jaewoong", "Lee Jaewoong", "イ・チェウン"],
    //     "이재환": ["Lee Jaehwan", "Lee Jaehwan", "Lee Jaehwan", "イ・チェファン"],
    //     "이정우": ["Lee Jungwoo", "Lee Jungwoo", "Lee Jungwoo", "イ・チョンウ"],
    //     "이정한": ["Lee Junghan", "Lee Junghan", "Lee Junghan", "イ・チョンハン"],
    //     "이종혁": ["Lee Jonghyuk", "Lee Jonghyuk", "Lee Jonghyuk", "イ・チョンヒョク"],
    //     "이창욱": ["Lee Changwook", "Lee Changwook", "Lee Changwook", "イ・チャンウク"],
    //     "이하늘": ["Lee Haneul", "Lee Haneul", "Lee Haneul", "イ・ハヌル"],
    //     "이하민": ["Lee Hamin", "Lee Hamin", "Lee Hamin", "イ・ハミン"],
    //     "임영준": ["Lim Youngjun", "Lim Youngjun", "Lim Youngjun", "イム・ヨンジュン"],
    //     "임회성": ["Lim Hoisung", "Lim Hoisung", "Lim Hoisung", "イム・フェソン"],
    //     "장진영": ["Jang Jinyoung", "Jang Jinyoung", "Jang Jinyoung", "チャン・チニョン"],
    //     "장혁재": ["Jang Hyeokjae", "Jang Hyeokjae", "Jang Hyeokjae", "jチャン・ヒョクチェp"],
    //     "장현준": ["Jang Hyeonjun", "Jang Hyeonjun", "Jang Hyeonjun", "チャン・ヒョンジュン"],
    //     "정대영": ["Chung Daeyoung", "Chung Daeyoung", "Chung Daeyoung", "チョン・テヨン"],
    //     "정민찬": ["Jeong Minchan", "Jeong Minchan", "Jeong Minchan", "チョン・ミンチャン"],
    //     "정성현": ["Jung Seonghyeon", "Jung Seonghyeon", "Jung Seonghyeon", "チョン・ソンヒョン"],
    //     "정승원": ["Jeong Seungwon", "Jeong Seungwon", "Jeong Seungwon", "チョン・スンウォン"],
    //     "정인증": ["Jung Injeung", "Jung Injeung", "Jung Injeung", "チョン・インジュン"],
    //     "정찬유": ["Jung Chanyu", "Jung Chanyu", "Jung Chanyu", "チョン・チャニュ"],
    //     "정현준": ["Jung Hyunjun", "Jung Hyunjun", "Jung Hyunjun", "チョン・ヒョンジュン"],
    //     "제태영": ["Je Taeyoung", "Je Taeyoung", "Je Taeyoung", "チェ・テヨン"],
    //     "조희재": ["Jo Heejea", "Jo Heejea", "Jo Heejea", "チョ・ヒジェ"],
    //     "조희창": ["Cho Heechang", "Cho Heechang", "Cho Heechang", "チョ・ヒチャン"],
    //     "진준형": ["Jin Junhyung", "Jin Junhyung", "Jin Junhyung", "チン・チュンヒョン"],
    //     "차휘민": ["Cha Hwimin", "Cha Hwimin", "Cha Hwimin", "チャ・フィミン"],
    //     "최명근": ["Choi Myunggeun", "Choi Myunggeun", "Choi Myunggeun", "チェ・ミョングン"],
    //     "최민우": ["Choi Minwoo", "Choi Minwoo", "Choi Minwoo", "チェ・ミヌ"],
    //     "최준": ["Choi Jun", "Choi Jun", "Choi Jun", "チェ・チュン"],
    //     "최진혁": ["Choi Jinhyeok", "Choi Jinhyeok", "Choi Jinhyeok", "チェ・チンヒョク"],
    //     "표경찬": ["Pyo Kyeongchan", "Pyo Kyeongchan", "Pyo Kyeongchan", "ピョ・キョンチャン"],
    //     "허서우": ["Heo Seowoo", "Heo Seowoo", "Heo Seowoo", "ホ・ソウ"],
    //     "현정인": ["Hyun Jungin", "Hyun Jungin", "Hyun Jungin", "ヒョン・チョンイン"],
    //     "홍용준": ["Hong Yongjoon", "Hong Yongjoon", "Hong Yongjoon", "ホン・ヨンジュン"],
    //     "홍정찬": ["Hong Jeongchan", "Hong Jeongchan", "Hong Jeongchan", "ホン・チョンチャン"],
    //     "황문일": ["Hwang Munil", "Hwang Munil", "Hwang Munil", "ファン・ムニル"],
    //     "황원태": ["Hwang Wontae", "Hwang Wontae", "Hwang Wontae", "ファン・ウォンテ"],
    //     "황윤우": ["Hwang Yunwoo", "Hwang Yunwoo", "Hwang Yunwoo", "ファン・ユヌ"],
    //     "황인서": ["Hwang Inseo", "Hwang Inseo", "Hwang Inseo", "ファン・インソ"],
    //     "황준우": ["Hwang Junwoo", "Hwang Junwoo", "Hwang Junwoo", "ファン・チュヌ"],

    //     "곽민준": ["Kwak Minjoon", "Kwak Minjoon", "Kwak Minjoon", "クァク・ミンジュン"],
    //     "권민준": ["Gweon Minjun", "Gweon Minjun", "Gweon Minjun", "クォン・ミンジュン"],
    //     "김도윤2": ["Kim Doyoon", "Kim Doyoon", "Kim Doyoon", "キム・トユン"],
    //     "김도의": ["Kim Doui", "Kim Doui", "Kim Doui", "キム・トイ"],
    //     "김도헌": ["Kim Dohyun", "Kim Dohyun", "Kim Dohyun", "キム・トヒョン"],
    //     "김선우1": ["Kim Seonu", "Kim Seonu", "Kim Seonu", "キム・ソヌ"],
    //     "김선우2": ["Kim Sunwoo", "Kim Sunwoo", "Kim Sunwoo", "キム・ソヌ"],
    //     "김승연": ["Kim Seungyeon", "Kim Seungyeon", "Kim Seungyeon", "キム・スンヨン"],
    //     "김재원": ["Kim Jaewon", "Kim Jaewon", "Kim Jaewon", "キム・チェウォン"],
    //     "김재훈": ["Kim Jaehoon", "Kim Jaehoon", "Kim Jaehoon", "キム・チェフン"],
    //     "김태헌": ["Kim Taeheon", "Kim Taeheon", "Kim Taeheon", "キム・テヒョン"],
    //     "김하율": ["Kim Hayul", "Kim Hayul", "Kim Hayul", "キム・ハユル"],
    //     "나진우": ["Na Jinwoo", "Na Jinwoo", "Na Jinwoo", "ナ・チヌ"],
    //     "노유준": ["Noh Yujun", "Noh Yujun", "Noh Yujun", "ノ・ユジュン"],
    //     "노윤준": ["Noh Yunjun", "Noh Yunjun", "Noh Yunjun", "ノ・ユンジュン"],
    //     "도율한": ["Do Yulhan", "Do Yulhan", "Do Yulhan", "ト・ユルハン"],
    //     "문지후": ["Moon Jihoo", "Moon Jihoo", "Moon Jihoo", "ムン・チフ"],
    //     "박대규": ["Park Daekyu", "Park Daekyu", "Park Daekyu", "パク・テギュ"],
    //     "박주윤": ["Park Jooyun", "Park Jooyun", "Park Jooyun", "パク・チュユン"],
    //     "박준성2": ["Park Junseong", "Park Junseong", "Park Junseong", "パク・チュンソン"],
    //     "박준희": ["Park Junhee", "Park Junhee", "Park Junhee", "パク・チュンヒ"],
    //     "백창민": ["Baik Changmin", "Baik Changmin", "Baik Changmin", "ペク・チャンミン"],
    //     "손채민": ["Son Chaemin", "Son Chaemin", "Son Chaemin", "ソン・チェミン"],
    //     "신예찬": ["Shin Yechan", "Shin Yechan", "Shin Yechan", "シン・イェチャン"],
    //     "심서용": ["Shim Seoyong", "Shim Seoyong", "Shim Seoyong", "シム・ソヨン"],
    //     "양현욱": ["Yang Hyunwook", "Yang Hyunwook", "Yang Hyunwook", "ヤン・ヒョヌク"],
    //     "오승현": ["Oh Seounghyeon", "Oh Seounghyeon", "Oh Seounghyeon", "オ・スンヒョン"],
    //     "오하랑": ["Oh Harang", "Oh Harang", "Oh Harang", "オ・ハラン"],
    //     "왕규민": ["Wang Gyumin", "Wang Gyumin", "Wang Gyumin", "ワン・キュミン"],
    //     "우명철": ["Woo Myungchul", "Woo Myungchul", "Woo Myungchul", "ウ・ミョンチョル"],
    //     "윤기현": ["Yoon Gihyun", "Yoon Gihyun", "Yoon Gihyun", "ユン・キヒョン"],
    //     "윤준식": ["Yoon Junsik", "Yoon Junsik", "Yoon Junsik", "ユン・チュンシク"],
    //     "이동석": ["Lee Dongseok", "Lee Dongseok", "Lee Dongseok", "イ・トンソク"],
    //     "이원건": ["Lee Wongeon", "Lee Wongeon", "Lee Wongeon", "イ・ウォンクォン"],
    //     "이재희": ["Lee Jaehee", "Lee Jaehee", "Lee Jaehee", "イ・チェヒ"],
    //     "이주율": ["Lee Juyul", "Lee Juyul", "Lee Juyul", "イ・チュユル"],
    //     "이한빈": ["Lee Hanbin", "Lee Hanbin", "Lee Hanbin", "イ・ハンビン"],
    //     "장민재": ["Jang Minjae", "Jang Minjae", "Jang Minjae", "チャン・ミンジェ"],
    //     "장성주": ["Jang Seongju", "Jang Seongju", "Jang Seongju", "チャン・ソンジュ"],
    //     "전소울": ["Jeon Soul", "Jeon Soul", "Jeon Soul", "チョン・ソウル"],
    //     "전은찬": ["Jeon Eunchan", "Jeon Eunchan", "Jeon Eunchan", "チョン・ウンチャン"],
    //     "전채민": ["Jeon Chaemin", "Jeon Chaemin", "Jeon Chaemin", "チョン・チェミン"],
    //     "전태호": ["Jeon Taeho", "Jeon Taeho", "Jeon Taeho", "チョン・テホ"],
    //     "전희성": ["Jun Heesung", "Jun Heesung", "Jun Heesung", "チョン・ヒソン"],
    //     "정은율": ["Jung Eunyul", "Jung Eunyul", "Jung Eunyul", "チョン・ウニュル"],
    //     "정주안": ["Jung Juan", "Jung Juan", "Jung Juan", "チョン・チュアン"],
    //     "조영서": ["Cho Youngseo", "Cho Youngseo", "Cho Youngseo", "チョ・ヨンソ"],
    //     "주수현": ["Joo Soohyun", "Joo Soohyun", "Joo Soohyun", "チュ・スヒョン"],
    //     "최성수": ["Choi Seongsu", "Choi Seongsu", "Choi Seongsu", "チェ・ソンス"],
    //     "최시온": ["Choi Zion", "Choi Zion", "Choi Zion", "チェ・シオン"],
    //     "최주영": ["Choi Jooyoung", "Choi Jooyoung", "Choi Jooyoung", "チェ・チュヨン"],
    //     "최한결": ["Choi Hankyeol", "Choi Hankyeol", "Choi Hankyeol", "チェ・ハンギョル"],
    //     "황주영": ["Hwang Juyoung", "Hwang Juyoung", "Hwang Juyoung", "ファン・チュヨン"],
